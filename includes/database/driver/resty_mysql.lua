...
--[[ What to say when the binding is not reachable.

  `lua-resty-mysql` ships with OpenResty rather than being installed, so
  `module 'resty.mysql' not found` followed by a page of search paths almost
  never means what it says: it means the process is not OpenResty. There is
...
local function missing_binding()
  ...
  return 'lua-resty-mysql is not installed. It ships with OpenResty; an\n'
    .. 'install without it can add it with:\n'
    .. '  luarocks install lua-resty-mysql\n'
    .. 'More: https://github.com/ophal/core'
end

function M.connect(config)
  local loaded, mysql = pcall(require, 'resty.mysql')
  local db, err
